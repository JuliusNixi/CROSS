...
public abstract class FileHandler {
     ...
     * 
     * This is an utility function to append to a file without rewriting all the file.
     * 
     ...
     * 
     */
    public static synchronized void removeLastLine(File file) throws IOException, NullPointerException {
     ...
     * @throws IllegalArgumentException If the line number is less than 1 or null or if the line number is out of bounds of the file.
     * @throws RuntimeException If there's an error editing the line in the file.
     * 
     */
    public static synchronized void editLine(File file, Long lineNumber) throws NullPointerException, IllegalArgumentException, RuntimeException {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "rw")) {
            ...
            if (currentLine == lineNumber) {
                ...
                DBUsersInterface.increaseEmptyFileLines();

            } else {
                throw new IllegalArgumentException("Line number is out of bounds of file lines.");
            }

        }catch (Exception ex) {
            throw new RuntimeException("Error editing the line in the file.");
        }

    }
    /**
     * 
     * Remove the last char from the file.
     * 
     * This is an utility function to append new content to a file without rewriting all the file.
     * 
     * Synchronized because it's intended to be used in a multi-threaded environment.
     * 
     * @param file The file from which remove the last char.
     * 
     * @return The previous char before the removed one. Used to check if an updated user is the last user in the file, in this case the ',' is not needed.
     * 
     * @throws NullPointerException If the file is null.
     * @throws IOException If there's an error removing the last char from the file.
     * 
     */
    public static synchronized char removeLastChar(File file) throws NullPointerException, IOException {
        
        if (file == null) {
            throw new NullPointerException("File from which remove the last char cannot be null.");
        }
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "rw")) {
            ...
            throw new IOException("Error removing the last char from the file.");
        }
